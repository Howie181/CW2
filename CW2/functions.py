...
# Generates a non-uniform mesh where specified
def inflated_mesh(nx, ny, r,
                  length):  # nx, ny are number of cells in x and y-direciton respectively, r is the growth ratio of the inflation layers
    dx = np.zeros((ny, nx))
    dy = np.zeros((ny, nx))
...
def make_sp(nx, ny, dx, dy, north, east, south, west, udf, gam):
    ...
    if north != 'zeroGradient' and 'UDF':  # north face
        s_p[0, ny - 1, :] = -2 * gam * dx[ny - 1, :] / dy[ny - 1, :]
    if north == 'UDF':
        s_p[0, ny - 1, :] = udf  # -dx[ny-1,:]*h_f*(2*gam/dy[ny-1,:])/(h_f-(2*gam/dy[ny-1,:]))

    if east != 'zeroGradient':  # east face
        s_p[1, :, nx - 1] = -2 * gam * dy[:, nx - 1] / dx[:, nx - 1]  # east face
    if east == 'UDF':
        s_p[1, :, nx - 1] = udf

    if south != 'zeroGradient':
        s_p[2, 0, :] = -2 * gam * dx[0, :] / dy[0, :]  # south face
    if south == 'UDF':
        s_p[2, 0, :] = udf

    if west != 'zeroGradient':
        s_p[3, :, 0] = -2 * gam * dy[:, 0] / dx[:, 0]  # west face
    if west == 'UDF':
        s_p[3, :, 0] = udf
